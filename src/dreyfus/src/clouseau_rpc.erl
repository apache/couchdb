...
clouseau_major_vsn() ->
    case version() of
        ...
        {ok, null} -> null;
        {'EXIT', noconnection} -> throw("Clouseau node is not connected.")
    end.
...
-spec check_service(Service) -> {service(), liveness_status()} | no_return() when
    Service :: service().
...
check_services() ->
    case connected() of
        ...
        false ->
            throw("Clouseau node is not connected.")
    end.
