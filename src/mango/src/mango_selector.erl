...
match({[{<<"$elemMatch">>, _Arg}]}, Values, #ctx{} = Ctx) ->
    [#failure{op = elemMatch, type = bad_value, params = [Values], ctx = Ctx}];
...
match({[{<<"$allMatch">>, _Arg}]}, Values, #ctx{} = Ctx) ->
    [#failure{op = allMatch, type = bad_value, params = [Values], ctx = Ctx}];
...
format_op(Op, _, bad_value, [Value]) ->
    io_lib:format("operator $~p was invoked with a bad value: ~s", [
        Op, couch_util:json_encode(Value)
    ]);
...
%
% and then uncommenting the following functions and running the unit tests for
% this module via this command:
%
%       make eunit apps=mango suites=mango_selector
%
