...
% Get index status. Gather design docs and then send them all to the workers
% and they locally gather index info in parallel. We query live shard copies
% and merge the result for each index. Fields like size, seqs and counts are
% returned from one copy per range and updates_pending object is computed from
% all the responding copies.
%
...
% Args select the design docs like they do for _design_docs (start_key,
% end_key, limit, skip, keys). Also {max_ddocs, N} specifies the maximum number
% of ddocs. Specifying more than that will return an error. Use pagination to
% get more results. To help paginate we get one extra design doc past the end
% then we use it's doc ID as the bookmark (startkey) of the next page.
go(DbName, Types, #mrargs{} = Args, Opts) ->
    Max = couch_util:get_value(max_ddocs, Opts, infinity),
    case fabric:design_docs(DbName, fetch_args(Args, Max)) of
        {ok, Meta, Fetched} ->
            case page(Fetched, Args, Max) of
                {ok, DDocs, Bookmark} ->
                    {Ctx, Responses} = index_info(DbName, DDocs, Types),
                    ByType = order(merge(DDocs, Types, Responses, Ctx), Args),
                    {ok, [{bookmark, Bookmark} | Meta], ByType};
                {error, _} = Error ->
                    Error
            end;
        {error, _} = Error ->
...
% Fetch one more design doc to let user paginate easily
fetch_args(#mrargs{limit = Limit} = Args, Max) ->
    Args#mrargs{limit = page_size(Limit, Max) + 1}.

page_size(Limit, infinity) ->
    Limit;
page_size(Limit, Max) ->
    min(Limit, Max).

page(Fetched, #mrargs{limit = Limit, keys = Keys}, Max) ->
    Size = page_size(Limit, Max),
    case length(Fetched) > Size of
        true when Limit > Max ->
            {error, too_many_design_docs};
        true ->
            {Page, [Next | _]} = lists:split(Size, Fetched),
            {ok, Page, bookmark(Next, Keys)};
        false ->
            {ok, Fetched, null}
    end.

bookmark(_Next, Keys) when is_list(Keys) ->
    null;
bookmark(Next, undefined) ->
    ddoc_id(Next).
...
%
merge(DDocs, Types, Responses, Ctx) ->
...
merge_responses_test() ->
    ...
    [{view, [{Ident, {ok, {Props}}}]}] = merge([ddoc()], [view], Res, ctx()),
...
merge_responses_search_test() ->
    ...
    [{search, [{Ident, {ok, {Props}}}]}] = merge([ddoc()], [search], Res, ctx()),
...
merge_responses_nouveau_test() ->
    ...
    [{nouveau, [{Ident, {ok, Merged}}]}] = merge([ddoc()], [nouveau], Res, ctx()),
...
merge_responses_ddoc_error_test() ->
    ...
    ?assertEqual(
        ...
        merge([ddoc()], [view, search, nouveau], Res, ctx())
    ).
...
merge_responses_no_copies_test() ->
    ...
    ?assertEqual(Expected, merge([NoViews], [view, search], [], ctx())).
...
merge_responses_sorted_test() ->
    ...
    [{search, Entries}] = merge([ddoc(), DDocA], [search], Res, ctx()),
    ?assertEqual(
        [
...
page_test() ->
    ...
    DdB = {[{~"_id", ~"_design/b"}]},
    DdC = {[{~"_id", ~"_design/c"}]},
    Args = fun(L) -> #mrargs{limit = L} end,
    ?assertEqual(#mrargs{limit = 101}, fetch_args(Args(100), infinity)),
    ?assertEqual(#mrargs{limit = 3}, fetch_args(Args(100), 2)),
    ?assertEqual(#mrargs{limit = 2}, fetch_args(Args(1), 2)),
    ?assertEqual({ok, [DdA, DdB], null}, page([DdA, DdB], Args(2), infinity)),
    ?assertEqual({ok, [DdA, DdB], null}, page([DdA, DdB], Args(100), 2)),
    ?assertEqual({ok, [DdA, DdB], ~"_design/c"}, page([DdA, DdB, DdC], Args(2), infinity)),
    ?assertEqual({ok, [DdA, DdB], ~"_design/c"}, page([DdA, DdB, DdC], Args(2), 2)),
    ?assertEqual({error, too_many_design_docs}, page([DdA, DdB, DdC], Args(100), 2)),
    ?assertEqual({ok, [DdA], null}, page([DdA, DdB], #mrargs{limit = 1, keys = [x]}, 2)),
    ?assertEqual({ok, [], ~"_design/a"}, page([DdA], Args(0), 2)).
