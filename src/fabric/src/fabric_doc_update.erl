...
handle_message({rexi_DOWN, _, {_, NodeRef}, _}, _Worker = nil, #acc{} = Acc0) ->
...
handle_message({ok, Replies}, Worker, #acc{} = Acc0) ->
    ...
    case skip_message(Acc2) of
        {stop, Msg} ->
            {stop, Msg};
        {ok, Acc3} ->
            Acc4 = start_followers([Worker#shard.range], Acc3),
...
remove_conflicts([Doc | DocRest], [conflict | ReplyRest], #acc{} = Acc0) ->
    {GroupedDocs, WC} = lists:foldl(
        fun({Worker, Docs}, {DocAcc, WCAcc}) ->
            case lists:delete(Doc, Docs) of
                [] ->
                    {DocAcc, WCAcc - 1};
                Rest ->
                    {[{Worker, Rest} | DocAcc], WCAcc}
            end
        end,
        {[], Acc0#acc.waiting_count},
        Acc0#acc.grouped_docs
    ),
    Acc1 = Acc0#acc{grouped_docs = GroupedDocs, waiting_count = WC},
