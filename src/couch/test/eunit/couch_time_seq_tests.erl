...
new_test() ->
    ...
    ?assert(couch_time_seq:check(New)),
    TSeq1 = couch_time_seq:update(New, 1),
    ?assert(couch_time_seq:check(TSeq1)),
    Upgraded = couch_time_seq:new(New),
    ?assert(couch_time_seq:check(Upgraded)).

check_test() ->
    New = couch_time_seq:new(),
    ?assert(couch_time_seq:check(New)),
    ?assertNot(couch_time_seq:check(0)),
    ?assertNot(couch_time_seq:check([])),
    ?assertNot(couch_time_seq:check(#{})),
    ?assertNot(couch_time_seq:check(#{v => 1, bins => junk})).
...
seq_0_test() ->
    TSeq = couch_time_seq:new(),
    ZeroSeq = couch_time_seq:update(TSeq, test_time() + 42, 0),
    ?assertEqual(TSeq, ZeroSeq).
...
update_when_bins_are_full_test() ->
    ...
    TSeq2 = couch_time_seq:update(TSeq, TopTime + hours(2), TopSeq + 2),
    #{bins := Bins2} = TSeq2,
...
update_3_hours_8_times_test() ->
    ...
    Hist = couch_time_seq:histogram(TSeq),
...
before_test() ->
    ...
    ?assertEqual(42, couch_time_seq:since(TSeq, T + hours(3))),
    ?assertEqual(43, couch_time_seq:since(TSeq, T + hours(3) + 1)).

histogram_test() ->
    New = couch_time_seq:new(),
    ?assertEqual([], couch_time_seq:histogram(New)),
    T = test_time(),
    TSeq0 = couch_time_seq:update(New, T, 42),
    % 42 in the result is the jump from 0 to 42
    ?assertEqual(
        [
            [<<"3000-01-01T00:00:00Z">>, 42]
        ],
        couch_time_seq:histogram(TSeq0)
    ),
    TSeq = couch_time_seq:update(TSeq0, T + hours(3), 43),
    % The 1 is the jump from 42 to 43
    ?assertEqual(
        [
            [<<"3000-01-01T00:00:00Z">>, 42],
            [<<"3000-01-01T03:00:00Z">>, 1]
        ],
        couch_time_seq:histogram(TSeq)
    ).
...
update_cnt(N, TimeInc) ->
    update_cnt(N, test_time(), 1, TimeInc, couch_time_seq:new()).
...
prop_no_empty_bins_after_update() ->
    ?FORALL(
        TSeq,
        tseq_g(),
        begin
            #{bins := Bins} = TSeq,
            EmptyBins = [B || {_, 0} = B <- Bins],
            [] =:= EmptyBins
        end
    ).
