...
-callback incref(DbHandle::db_handle()) -> retry | {ok, NewDbHandle::db_handle()}.
-callback decref(DbHandle::db_handle()) -> ok.
-callback monitored_by(DbHande::db_handle()) -> [pid()].
-callback lock(DbHande::db_handle()) -> boolean().
-callback unlock(DbHande::db_handle()) -> true.
...
lock(#db{} = Db) ->
  #db{engine = {Engine, EngineState}} = Db,
  Engine:lock(EngineState).

unlock(#db{} = Db) ->
  #db{engine = {Engine, EngineState}} = Db,
  Engine:unlock(EngineState).
