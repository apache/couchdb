...
%
%   1752724800 = 2025-08-01T00:00:00Z
%
...
check(#{v := ?VER, bins := Bins}) when is_list(Bins) ->
    ...
    % should accept old versions as well (return true).
    true;
check(_) ->
    false.
...
% Return a highest known sequence that comes before the given time. If the time
...
rollup(TimeNow, Bins) ->
    ...
    % chronological order (reversed) and we know their lengths. So, as a
    % preparatory step, reverse the bins and make them into 3-tuples that look
    % like {Time, Length, Seq}.
...
do_rollup([_ | _] = Bins, _TimeNow, [_] = _Intervals) ->
    ...
    % succeed just resample the oldest half. Using the max(bin lengths), we're
...
merge([{T1, L1, S1}, {_, L2, _} | Rest], Int, Acc) when L1 =< Int, L2 =< Int ->
    Merged = {T1, L1 + L2, S1},
...
% The length will be the middle element [{Time, Len, Seq}, ...]
%
