        ...
        {ok, Token} ->
            {
                ...
                update_used_since_acquisition(WorkerState)
            };
        {error, Reason} ->
...
get_token(#worker_state{} = WorkerState) ->
    ...
    case ets:lookup(?PUBLIC, APIKeyMAC) of
        [#public_entry{token = Token}] when is_binary(Token) ->
            {ok, Token};
        [#public_entry{}] ->
            gen_server:call(?MODULE, {get_token, APIKeyMAC}, request_timeout_ms());
        [] ->
            {error, no_token}
    end.

update_used_since_acquisition(#worker_state{} = WorkerState) ->
    ...
    case Now - LastUsed > ?MINUTE of
        true ->
            ?MODULE ! {used_since_acquisition, APIKeyMAC},
...
init(_) ->
    case token_url() of
        {ok, TokenUrl} ->
            ...
            erlang:send_after(check_interval_ms(), self(), refresh_expire_check),
            {ok, #state{
...
handle_info({used_since_acquisition, APIKeyMAC}, State) ->
    ets:update_element(?PRIVATE, APIKeyMAC, {#private_entry.used_since_acquisition, true}),
    {noreply, State};
handle_info({refresh_token, APIKeyMAC}, State) ->
    case ets:lookup(?PRIVATE, APIKeyMAC) of
        ...
        [#private_entry{acquire_mref = undefined} = Entry] ->
            case Entry#private_entry.used_since_acquisition of
                true ->
                ...
                false ->
                    %% let it expire and free any waiters
                    reply_and_reset(Entry, {error, token_expiring})
            end;
        [#private_entry{}] ->
            ok
    end,
...
handle_info({expire_api_key_entry, APIKeyMAC}, State) ->
    case ets:lookup(?PRIVATE, APIKeyMAC) of
        ...
        [#private_entry{} = Entry] ->
            ...
            if
                Entry#private_entry.acquire_mref /= undefined ->
                    demonitor(Entry#private_entry.acquire_mref, [flush]);
                true ->
                    ok
            end,
            reply_all(Entry#private_entry.waiters, {error, expired_api_key_entry})
    end,
    {noreply, State};
handle_info(refresh_expire_check, #state{} = State) ->
    NowSecs = now_secs(),
    EarlyRefreshSecs = early_refresh_secs(),
    Matches = ets:select(?PRIVATE, [
        {
            #private_entry{api_key_mac = '$1', expires_at_secs = '$2', _ = '_'},
            [{is_integer, '$2'}, {'=<', '$2', NowSecs + EarlyRefreshSecs}],
            [['$1', '$2']]
        }
    ]),
    lists:foreach(
        fun([APIKeyMAC, ExpiresAtSecs]) ->
            if
                NowSecs >= ExpiresAtSecs ->
                    self() ! {expire_api_key_entry, APIKeyMAC};
                true ->
                    erlang:send_after(jitter_ms(), self(), {refresh_token, APIKeyMAC})
            end
        end,
        Matches
    ),
            ...
            case StatusCode of
                200 ->
                    case decode_iam_response(ResponseBody) of
                        {ok, Token, ExpiresAtSecs} ->
                            UUID = api_key_uuid(Token),
                            couch_log:notice("~p: refreshed api key ~s", [
                                ?MODULE, UUID
                            ]),
                            true = ets:update_element(?PUBLIC, Entry#private_entry.api_key_mac, {
                                #public_entry.token, Token
                            }),
                            reply_and_reset(
                                Entry#private_entry{
                                    api_key_uuid = UUID,
                                    expires_at_secs = ExpiresAtSecs,
                                    used_since_acquisition = false
                                },
                                {ok, Token}
                            );
...
handle_config_change("replicator_auth_ibm", "token_url", _, _, State) ->
...
acquire_token(APIKey, #state{token_url = Url}) ->
    ...
    {_Pid, Ref} = spawn_monitor(fun() ->
        exit(
            try
                post(Url, Headers, Body, request_timeout_ms())
            catch
...
decode_iam_response(ResponseBody) ->
    try jiffy:decode(ResponseBody, [return_maps]) of
        Decoded when is_map(Decoded) ->
            ...
            ExpiresAtSecs = maps:get(~"expiration", Decoded, undefined),
            if
                is_binary(Token) andalso is_integer(ExpiresAtSecs) ->
                    {ok, Token, ExpiresAtSecs};
                true ->
...
token_url() ->
    Url = config:get(
        "replicator_auth_ibm", "token_url", "https://iam.cloud.ibm.com/identity/token"
    ),
...
reply_and_reset(#private_entry{} = Entry, Reply) ->
    ...
    reply_all(Entry#private_entry.waiters, Reply).

-spec reply_all(Waiters :: [gen_server:from()], Reply :: term()) -> ok.
...
request_timeout_ms() ->
    max(0, 1000 * config:get_integer("replicator_auth_ibm", "request_timeout_secs", 30)).

check_interval_ms() ->
    max(0, 1000 * config:get_integer("replicator_auth_ibm", "check_interval_secs", 60)).

early_refresh_secs() ->
    max(0, config:get_integer("replicator_auth_ibm", "early_refresh_secs", 300)).

jitter_ms() ->
    JitterMS = max(0, 1000 * config:get_integer("replicator_auth_ibm", "jitter_secs", 60)),
    case JitterMS of
        0 -> 0;
        _ -> rand:uniform(JitterMS)
    end.

retry_ms() ->
    max(0, 1000 * config:get_integer("replicator_auth_ibm", "retry_secs", 10)).
...
test_refresh_token(_) ->
    ?_test(
        begin
            ...
            wait_for_refresh(APIKeyMAC, Token1, 20),
...
token_url_test() ->
    try
        meck:expect(config, get, fun("replicator_auth_ibm", "token_url", _) ->
            "https://nohost.invalid"
        end),
        ?assertEqual({ok, "https://nohost.invalid"}, token_url()),
        meck:expect(config, get, fun("replicator_auth_ibm", "token_url", _) -> "https://" end),
...
mock_config(Port) ->
    ...
    meck:expect(config, get_integer, fun
        ("replicator_auth_ibm", "check_interval_secs", _) -> 1;
        ("replicator_auth_ibm", "early_refresh_secs", _) -> 1;
        ("replicator_auth_ibm", "jitter_secs", _) -> 0;
        ("replicator_auth_ibm", "retry_secs", _) -> 1;
