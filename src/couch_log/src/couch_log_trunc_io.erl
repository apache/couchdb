...
format_test() ->
    ...
    ?assertEqual(
        ...
        lists:flatten(format("~w", [["foo", $b, $a, $r]], 50))
    ),
    ...
    ?assertEqual(
        "[\"foo\",98,97,114]",
        lists:flatten(format("~22P", [["foo", $b, $a, $r], 10], 50))
    ),
    ?assertEqual("**********", lists:flatten(format("~10W", [["foo", $b, $a, $r], 10], 50))),
    ?assertEqual(
        "[[102,111,111],98,97,114]",
        lists:flatten(format("~25W", [["foo", $b, $a, $r], 10], 50))
    ),
    ...
    ?assertEqual(
        "[\"foo\",98,97,114]",
        lists:flatten(format("~10P", [["foo", $b, $a, $r], 10], 50))
    ),
    ok.
...
binary_printing_test() ->
    ...
    ?assertEqual(
        "<<\"\\\"hello world\\\"\">>",
        lists:flatten(format("~p", [<<"\"hello world\"">>], 50))
    ),
    ?assertEqual("<<\"hello\\\\world\">>", lists:flatten(format("~p", [<<"hello\\world">>], 50))),
    ?assertEqual("<<\"hello\\\\\world\">>", lists:flatten(format("~p", [<<"hello\\\world">>], 50))),
    ?assertEqual(
...
map_printing_test() ->
    case erlang:is_builtin(erlang, is_map, 1) of
        true ->
            ...
            ?assertEqual(
                "#{bar => ...}",
                lists:flatten(format("~p", [maps:from_list([{bar, foo}])], 7))
            ),
            ?assertEqual(
                "#{bar => ...}",
                lists:flatten(format("~p", [maps:from_list([{bar, foo}])], 9))
            ),
            ?assertEqual(
                "#{bar => foo}",
                lists:flatten(format("~p", [maps:from_list([{bar, foo}])], 10))
            ),
            ?assertEqual(
                "#{bar => ...,...}",
...
depth_limit_test() ->
    ...
    ?assertEqual(
        "<<\"abcd\"...>>",
        lists:flatten(format("~P", [<<$a, $b, $c, $d, $e, 0>>, 2], 50))
    ),
