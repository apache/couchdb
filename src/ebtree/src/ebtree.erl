    ...
    case Node#node.prev /= undefined andalso collate(Tree, StartKey, FirstKey, [lt, eq]) of
        true ->
            reverse_range(
                Tx,
                Tree,
                get_node(Tx, Tree, Node#node.prev),
                StartKey,
                EndKey,
                AccFun,
                Acc1
...
group_reduce_int_test_() ->
    ...
    [
        ...
        ?_test(
            ?assertEqual(
                [{null, 99}],
                group_reduce(Db, Tree, 2, ebtree:max(), GroupKeyFun, UserAccFun, [])
            )
        ),
        ?_test(
...
range_test_() ->
    {timeout, 1000, fun() ->
        ...
        Keys = [X || {_, X} <- lists:sort([{rand:uniform(), N} || N <- lists:seq(1, Max)])],
        Tree = lists:foldl(
            fun(Key, T) -> insert(Db, T, Key, Key + 1) end,
            open(Db, <<1, 2, 3>>, 10),
            Keys
        ),
        lists:foreach(
