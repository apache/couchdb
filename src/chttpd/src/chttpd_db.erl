...
% `type=view|search|nouveau` as a filtering parameter as well as all the
...
% Since we're gathering index info based on design docs, total_rows counts
% design docs. The bookmark is the doc ID of the design doc after the page so
% it can be used as the startkey of the next page, or null when there is none,
% like for a `keys` request, for example.
meta_to_json(Meta) ->
    [
        {total_rows, couch_util:get_value(total, Meta, null)},
        {bookmark, couch_util:get_value(bookmark, Meta, null)}
    ].
...
% index info is successfully returned there is an "ok": true entry. If it
% couldn't be returned (all copies are not responding or the index subsystem is
...
meta_to_json_test_() ->
    [
        ?_assertEqual(
            [{total_rows, 3}, {bookmark, ~"_design/d"}],
            meta_to_json([{bookmark, ~"_design/d"}, {total, 3}, {offset, 1}])
        ),
        ?_assertEqual(
            [{total_rows, 3}, {bookmark, null}], meta_to_json([{bookmark, null}, {total, 3}])
        ),
        ?_assertEqual([{total_rows, null}, {bookmark, null}], meta_to_json([]))
    ].
