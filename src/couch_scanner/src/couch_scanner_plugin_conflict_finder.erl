...
check(#st{doc_report = true, opts = Opts} = St, DbName, DocId, Revs) ->
    ...
    DbReport = maps:map(fun(_K, V) when is_list(V) -> length(V) end, DocReport),
...
gen_report(doc, ?CONFLICTS, #{?CONFLICTS := true}, Revs) ->
    #{?CONFLICTS => [couch_doc:rev_to_str(R#rev_info.rev) || R <- Revs]};
gen_report(doc, ?DELETED_CONFLICTS, #{?DELETED_CONFLICTS := true}, Revs) ->
    #{?DELETED_CONFLICTS => [couch_doc:rev_to_str(R#rev_info.rev) || R <- Revs]};
