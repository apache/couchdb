defmodule WithoutQuorumTest do
  ...
  @moduletag :test_cluster_without_quorum
  ...
  test "Creating/Updating/Deleting doc should return 202-Acepted", context do
   ...
   rev = resp.body["_rev"]
   resp = Couch.put("/#{context[:db_name]}/0", body: %{:_id => "0",:_rev => rev, :a => 2})
   msg = "Should return 202-Acepted"
   assert resp.status_code == 202,msg
  ...
  @tag :with_db_name
  test "Creating/Updating/Deleting doc with overriden quorum should return 201-Created", context do
   db_name = context[:db_name]
   Couch.put("/#{db_name}")

   resp = Couch.post("/#{context[:db_name]}",
                     query: %{:w => 1},
                     body: %{:_id => "0", :a => 1})
   ...
   resp = Couch.get("/#{context[:db_name]}/0")
   rev = resp.body["_rev"]
   resp = Couch.put("/#{context[:db_name]}/0",
                    query: %{:w => 1},
  ...
  test "Attachments should return 202-Acepted", context do
   ...
   resp = Couch.put("/#{context[:db_name]}/0/foo.txt",
            query: %{:rev => rev},
  ...
  test "Attachments overriden quorum should return 201-Created", context do
   ...
   rev = resp.body["rev"]
   resp = Couch.delete("/#{context[:db_name]}/0/foo.txt",
                    query: %{:rev => rev, :w =>1})
   msg = "Should return 200-Ok"
   assert resp.status_code == 200,msg
